...
export interface OrganizeNoteParams {
  conversationId: string;
  noteId?: string | null;
}

export interface OrganizeNoteResponse extends NoteResponse {
  organized: true;
}

export const listNotes = (conversationId?: string): Promise<NoteResponse> => {
  const url = conversationId
    ? `${endpoints.notes()}?conversationId=${encodeURIComponent(conversationId)}`
    : endpoints.notes();
  return request.get(url);
};

export const getNote = (noteId: string): Promise<NoteResponse> => {
  return request.get(endpoints.note(noteId));
};

export const createNote = (params: CreateNoteParams): Promise<NoteResponse> => {
  return request.post(endpoints.notes(), params);
};

export const updateNote = (params: UpdateNoteParams): Promise<NoteResponse> => {
  const { noteId, ...body } = params;
  return request.put(endpoints.note(noteId), body);
};
...
export const moveNote = (
  ...
  return request.patch(endpoints.noteMove(noteId), { parentId, sortOrder });
};

export const deleteNote = (noteId: string): Promise<NoteResponse> => {
  return request.delete(endpoints.note(noteId));
};

export const getLearningState = (conversationId: string): Promise<LearningStateResponse> => {
  return request.get(
    `${endpoints.noteLearningState()}?conversationId=${encodeURIComponent(conversationId)}`,
  );
};
...
export const updateLearningState = (
  ...
  return request.put(endpoints.noteLearningState(), {
    conversationId,
    learningState,
  });
};

export const organizeNote = (params: OrganizeNoteParams): Promise<OrganizeNoteResponse> => {
  return request.post(endpoints.noteOrganize(), params);
};
