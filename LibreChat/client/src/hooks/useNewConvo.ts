...
function injectRootPrompt<T extends Partial<TPreset> | Partial<TConversation> | null | undefined>(
  value: T,
): T {
  if (!value) {
    return value;
  ...
  return {
    ...value,
    promptPrefix: mergeRootPrompt(value.promptPrefix),
  } as T;
}
...
const useNewConvo = (index = 0) => {
      ...
      async (
        ...
        const activePreset =
          ...
          buildDefaultConversation
            ? defaultPreset
            : preset;
        const resolvedPreset = injectRootPrompt(activePreset);

        const disableParams =
          _disableParams ??
          (resolvedPreset?.presetId != null &&
            resolvedPreset.presetId &&
            resolvedPreset.presetId === defaultPreset?.presetId);

        if (buildDefaultConversation) {
          let defaultEndpoint = getDefaultEndpoint({
            convoSetup: resolvedPreset ?? conversation,
            endpointsConfig,
          });
          ...
          conversation = buildDefaultConvo({
            conversation,
            lastConversationSetup: resolvedPreset as TConversation,
            endpoint: defaultEndpoint,
            models,
        ...
        conversation.promptPrefix = mergeRootPrompt(conversation.promptPrefix);
  ...
  const newConversation = useCallback(
    function createNewConvo({
      ...
      let preset = injectRootPrompt(_preset);
      ...
      if (
        ...
        preset = injectRootPrompt(getModelSpecPreset(defaultModelSpec));
      }
