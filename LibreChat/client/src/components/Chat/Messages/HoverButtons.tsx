...
import { useOrganizeNoteMutation } from '~/data-provider/Notes';
...
const HoverButtons = ({
  ...
  const selectedNoteId = useRecoilValue(store.selectedNoteId);
  const setSelectedNoteId = useSetRecoilState(store.selectedNoteId);
  const organizeNoteMutation = useOrganizeNoteMutation();
  ...
  const handleOrganizeNote = async () => {
    ...
    if (!conversationId || organizeNoteMutation.isLoading) {
      return;
    }

    try {
      const result = await organizeNoteMutation.mutateAsync({
        conversationId,
        noteId: selectedNoteId,
      });

      if (result.note?._id) {
        setSelectedNoteId(result.note._id);
      }
    } catch {
      // Mutation state is enough for now; avoid throwing into the render tree.
    }
  };
  ...
  return (
    <div className="group visible flex justify-center gap-0.5 self-end focus-within:outline-none lg:justify-start">
      ...
      <HoverButton
        ...
        title={
          organizeNoteMutation.isLoading
            ? localize('com_ui_organizing_note')
            : selectedNoteId
              ? localize('com_ui_organize_to_note')
              : localize('com_ui_organize_to_new_note')
        }
        icon={
          organizeNoteMutation.isLoading ? (
        ...
        isDisabled={organizeNoteMutation.isLoading}
        isLast={isLast}
        className="active"
      />
