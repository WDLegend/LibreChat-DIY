...
import FunctionPlot from '~/components/Messages/Content/FunctionPlot';
import LearningMap from '~/components/Messages/Content/LearningMap';
...
export const code: React.ElementType = memo(function MarkdownCode({
  ...
  const isMath = normalizedLang === 'math';
  const isMermaid = normalizedLang === 'mermaid';
  const isFunction = normalizedLang === 'function';
  const isLearningMap =
    normalizedLang === 'learning-tree' ||
    normalizedLang === 'learning_tree' ||
    normalizedLang === 'learningtree' ||
    normalizedLang === 'learning-map' ||
    normalizedLang === 'learning_map' ||
    normalizedLang === 'learningmap';
  const isSingleLine = typeof children === 'string' && children.split('\n').length === 1;

  const { getNextIndex, resetCounter } = useCodeBlockContext();
  const blockIndex = useRef(
    getNextIndex(isMath || isMermaid || isFunction || isLearningMap || isSingleLine),
  ).current;
  ...
  if (isMath) {
    ...
    const content = typeof children === 'string' ? children : String(children);
    return <FunctionPlot>{content}</FunctionPlot>;
  } else if (isLearningMap) {
    const content = typeof children === 'string' ? children : String(children);
    return <LearningMap>{content}</LearningMap>;
